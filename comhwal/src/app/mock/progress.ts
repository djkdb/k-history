 ...
 * 풀던 시험 이어 하기.
 *
 * 시험 도중에 전화가 오거나 앱이 내려가면 40분 동안 푼 것이 통째로
 * 날아간다. 그것만은 막아야 해서, 답을 고를 때마다 여기에 적어 둔다.
 *
 * 학습 기록(comhwal-state)과 따로 두는 이유는 성격이 달라서다.
 * 이건 "지금 풀고 있는 중"이라는 임시 상태이고, 제출하면 사라진다.
 * 학습 기록에 섞으면 매 문항마다 본 저장소를 건드리게 된다.
 */
export const PROGRESS_KEY = "comhwal:mock-progress";
