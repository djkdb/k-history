 ...
 * 것이 통째로 날아갔다. 모의고사는 이어 풀 수 있는데 퀴즈만 못 할
 * 이유가 없다.
 *
 * 문제 자체를 저장하지 않고 만들어 낸 값(seed)만 적어 둔다. 같은 seed로
 * 다시 만들면 같은 문제가 같은 순서로 나오기 때문이다. 저장 용량이
 * 훨씬 작고, 무엇보다 학습 기록(comhwal-state)을 건드리지 않는다.
 *
 * 채점은 답을 고르는 순간 이미 끝나 기록에 반영된다. 그래서 여기 남는
 * 것은 "어디까지 왔나"일 뿐이고, 이어 하지 않고 버려도 점수는 잃지 않는다.
 */
export const QUIZ_PROGRESS_KEY = "comhwal:quiz-progress";
