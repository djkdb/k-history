...
/** 복습 카드 하나가 가리키는 것 — 개념일 수도, 수식일 수도, 단축키일 수도 있다 */
type Item = {
  id: string;
  kind: "concept" | "formula" | "shortcut";
  front: string;
  back: string;
  extra?: string;
  href?: string;
      ...
      <div className="pt-6">
        ...
        <Card className="mt-3 min-h-44">
          ...
          {flipped ? (
            ...
            >
              {item.kind === "shortcut" ? (
                <KeyCaps combo={item.back} size="lg" />
              ) : (
                <p
                  className={cn(
                    "text-[15px] leading-[1.9] text-zinc-200",
                    item.kind === "formula" && "mono font-bold",
                  )}
                >
                  {item.back}
                </p>
    ...
    return (
      <div className="pt-8 text-center">
        ...
        <p className="mt-4 text-[13px] leading-relaxed text-zinc-500">
          몰랐던 것은 곧바로 다시 대기열에 올라갑니다. 알았던 것은 한 단계
          뒤로 미뤄져 잊을 때쯤 다시 나타납니다.
        </p>
  ...
  return (
    <div className="pt-6">
      ...
      <p className="mt-1 text-sm leading-relaxed text-zinc-400">
        오늘 · 1일 · 3일 · 7일 · 14일 · 30일 간격으로 다시 물어봅니다. 잊기
        직전에 보는 것이 가장 오래 남습니다.
      </p>
