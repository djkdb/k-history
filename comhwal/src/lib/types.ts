...
export interface MockAttempt {
  ...
  // ─── 오답 노트를 위해 남기는 것들 ─────────────────────────────
  // 아래 넷은 모두 선택 항목이다. 이 기능이 생기기 전에 본 시험에는
  // 없고, 없으면 없는 대로 보여 준다. (있는 기록을 버리지 않는다)

  /**
   * 시험지를 다시 만들어 낼 값.
   *
   * 문항 60개를 통째로 저장하지 않는 이유는, 같은 seed로 다시 만들면
   * 같은 문제가 같은 순서로 나오기 때문이다. 저장은 수십 바이트로 끝난다.
   */
  seed?: number;
  /**
   * 그때 나온 문항 id 순서.
   *
   * seed만 믿을 수 없는 이유는 개념 데이터가 늘거나 고쳐지면 같은 seed에서
   * 다른 시험지가 나오기 때문이다. 다시 만든 시험지의 id가 이것과 한 글자도
   * 다르지 않을 때만 문항별 오답 노트를 보여 준다.
   */
  qids?: string[];
  /**
   * 이 답들이 어느 판에서 매겨졌는가 (mock/progress.ts 의 FORMAT 과 같은 값).
   * 보기를 섞는 방식이 바뀌면 저장된 "몇 번을 골랐나"가 다른 보기를 가리킨다.
   */
  fmt?: number;
  /** 틀린 문항의 출처 id (개념·수식·단축키). 시험지를 못 만들 때의 대비책 */
