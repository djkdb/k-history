...
import { EXAM, SUBJECTS, SUBJECT_MAP, cutoff, judge } from "@/data/exam";
...
export default function MockPage() {
  ...
  return (
    <div className="pt-6">
      <h1 className="text-xl font-bold tracking-tight">모의고사</h1>
      <p className="mt-1 text-sm leading-relaxed text-zinc-400">
        {EXAM.questions}문항 · {EXAM.minutes}분 · 문항당 {EXAM.perQuestion}점.
        실제 시험과 같은 구성입니다.
      </p>
      ...
      {resume && (
        <Card className="mt-4 border-amber-500/30 bg-amber-500/[0.08]">
          ...
          <p className="mt-1 text-[13px] leading-relaxed text-zinc-300">
            {Object.keys(resume.answers).length}문항까지 답했고, 남은 시간은 약{" "}
            {Math.max(0, Math.round((resume.endsAt - Date.now()) / 60000))}분입니다.
          </p>
      ...
      <Card className="mt-3 border-rose-500/25 bg-rose-500/[0.06]">
        <div className="flex items-start gap-2">
          ...
          <p className="text-[13px] leading-relaxed text-zinc-300">
            SQLD 는 <b>총점 {EXAM.passScore}점 이상</b>이어야 하고, 여기에 더해
            <b> 과목별 40% 미만이면 과락</b>입니다. 1과목은 {cutoff("modeling")}문항,
            2과목은 {cutoff("sql")}문항이 그 선입니다. 그래서 결과 화면에 과목별
            점수를 따로 보여 드립니다.
          </p>
        </div>
      </Card>
      ...
      {attempts.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {[...attempts].reverse().map((a) => {
            const v = judge(a.bySubject);
            return (
              <Card key={a.startedAt}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-bold">
                      {new Date(a.startedAt).toLocaleDateString("ko-KR")} 응시
                    </p>
                    <p className="mt-0.5 text-[11px] text-zinc-500">
                      {a.correct} / {a.total}문항 ·{" "}
                      {Math.round((a.finishedAt - a.startedAt) / 60000)}분 소요
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-lg font-bold">{a.score}점</p>
                    <p
                      className={cn(
                        "text-[11px] font-bold",
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {a.bySubject.map((b) => {
                    const bad = b.total > 0 && b.correct / b.total < EXAM.cutRatio;
                    return (
                      <span
                        key={b.subject}
                        className={cn(
                      ...
                      >
                        {SUBJECT_MAP[b.subject]?.short ?? b.subject} {b.correct}/
                        {b.total}
                      </span>
                    );
                  })}
