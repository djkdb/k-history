...
export interface MockAttempt {
   ...
   * 시험지를 다시 만들어 낼 값.
   *
   * 문항 백 개를 통째로 저장하지 않는 이유는, 같은 seed 로 다시 만들면
   * 같은 문항이 같은 순서로 나오기 때문이다. 저장은 수십 바이트로 끝난다.
   */
  seed?: number;
  /**
   * 그때 나온 문항 id 순서.
   *
   * seed 만 믿을 수 없는 이유는 문항이 늘거나 고쳐지면 같은 seed 에서
   * 다른 시험지가 나오기 때문이다. 다시 만든 시험지의 id 가 이것과 한
   * 글자도 다르지 않을 때만 문항별 오답 노트를 보여 준다.
   */
  qids?: string[];
  /** 이 답들이 어느 판에서 매겨졌는가 (mock/progress.ts 의 MOCK_FORMAT) */
  fmt?: number;
  /** 틀린 문항 id — 시험지를 못 만들 때의 대비책 */
  wrongIds?: string[];
}
